...
def plot_image_grid(name, images_np, interpolation='lanczos', output_path="output/"):
    ...
    images_np[0] = OD_to_RGB(images_np[0])/255.
    images_np[1] = OD_to_RGB(images_np[1])/255.
...
def pil_to_np_norm(img_PIL, with_transpose=True):
    """
    Converts image in PIL format to np.array.
